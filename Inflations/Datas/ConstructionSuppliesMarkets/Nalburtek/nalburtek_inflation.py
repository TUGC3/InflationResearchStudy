"""
inflation.py — Nalburtek Daily Inflation Calculator

Computes three inflation metrics for Nalburtek products:
...
# ── Directories ───────────────────────────────────────────────────────────────
DATA_DIR = _PROJECT_ROOT / "InflationItems" / "Datas" / "ConstructionSuppliesMarkets" / "Nalburtek"
INFLATION_OUT_DIR = _PROJECT_ROOT / "Inflations" / "Datas" / "ConstructionSuppliesMarkets" / "Nalburtek"

def nalburtek_category_to_tuik(url):
    """Map Nalburtek product (by URL) to TUIK main group code.
    Construction and hardware supplies mainly fall under TUIK group 04 (Housing).
    """
    return "04"

def _load_csv(date_str):
    """Load a Nalburtek daily CSV by date string (YYYY-MM-DD), return DataFrame or None."""
    try:
        fpath = DATA_DIR / f"Nalburtek_Products_{date_str}.csv"
        ...
        df['cleaned_price'] = pd.to_numeric(df['SalePrice'], errors='coerce')
        return df
...
def _compute_metrics(df_current, df_past):
    ...
    df_current['tuik_category'] = df_current['URL'].apply(nalburtek_category_to_tuik)

    # Use URL for matching as StockCode can contain messy UI text
    past_subset = df_past[['URL', 'cleaned_price']].rename(columns={'cleaned_price': 'past_price'})
    merged = df_current.merge(past_subset, on='URL', how='left')
...
def calculate_inflation(target_date=None, compare_date=None):
    ...
    detail_base['tuik_category'] = detail_base['URL'].apply(nalburtek_category_to_tuik)
    ...
    for label, past_str in intervals.items():
        ...
        detail_base = detail_base.merge(
            merged[['URL', 'basic_inflation']].rename(columns={'basic_inflation': f'basic_inflation_{label}'}),
            on='URL', how='left'
        )
    ...
    # Save detailed
    detail_file = INFLATION_OUT_DIR / f"nalburtek_inflation_{today_str}.csv"
