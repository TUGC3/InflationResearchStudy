"""
inflation.py — Golden Rose daily inflation calculator.

Computes per-product inflation, simple average inflation, and TUIK-weighted
inflation for Golden Rose catalog snapshots.
"""
...
from tuik_config import goldenrose_category_to_tuik, normalised_weights
...
try:
    ...
    _spec = importlib.util.spec_from_file_location(
        "goldenrose_scraper_config",
        _config_path,
    )
    if _spec is None or _spec.loader is None:
        raise ImportError(f"Could not load scraper config from {_config_path}")
    _config = importlib.util.module_from_spec(_spec)
    _spec.loader.exec_module(_config)
    DATA_DIR = Path(_config.OUTPUT_DIR)
except Exception:
    DATA_DIR = _PROJECT_ROOT / "InflationItems" / "Datas" / "Cosmetics" / "GoldenRose"

INFLATION_OUT_DIR = _CODES_DIR.parent / "Datas" / "Cosmetics" / "GoldenRose"
...
def _load_csv(date_str):
    """Load one Golden Rose daily snapshot for the given date."""
    fpath = DATA_DIR / f"goldenrose_{date_str}.csv"
    if not fpath.exists():
...
def _add_tuik_category(df):
    ...
    enriched["tuik_category"] = enriched.apply(
        lambda row: goldenrose_category_to_tuik(
...
def calculate_inflation(target_date=None, compare_date=None):
    """Calculate inflation outputs for Golden Rose."""
    base_date = (
    ...
    detail_file = INFLATION_OUT_DIR / f"goldenrose_inflation_{today_str}.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Golden Rose inflation calculator")
