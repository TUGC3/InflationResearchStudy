"""
inflation.py — Rossmann Daily Inflation Calculator
...
- Maps 7 Rossmann navigation categories to 3 TUIK groups (05, 06, 12)
- Outputs detailed per-product data and store-level summaries
- Handles missing historical data gracefully

Rossmann TUIK mapping (see ``tuik_config.py``):
  - Makyaj / Cilt Bakımı / Kişisel Bakım / Anne & Bebek → 12 (Kişisel bakım)
  - Sağlık & Gıda → 06 (Sağlık)
  - Temizlik / Ev & Yaşam → 05 (Mobilya, ev aletleri ve ev bakım)

Output Files
------------
- ``rossmann_inflation_YYYY-MM-DD.csv`` – per-product detail file
- ``inflation_summary.csv``              – store-level summary, append-only

Both live in ``Inflations/Datas/Cosmetics/Rossmann/``.
...
try:
    _scraper_dir = (
        _PROJECT_ROOT
        / "InflationItems" / "Codes" / "Cosmetics" / "Rossmann" / "scripts"
    )
    ...
    DATA_DIR = (
        _PROJECT_ROOT / "InflationItems" / "Datas" / "Cosmetics" / "Rossmann"
    )

logger = logging.getLogger(__name__)

# ── Output directory ──────────────────────────────────────────────────────────
INFLATION_OUT_DIR = _CODES_DIR.parent / "Datas" / "Cosmetics" / "Rossmann"


def _load_csv(date_str: str):
    """Load a Rossmann daily CSV by date string, return DataFrame or None."""
    fpath = DATA_DIR / f"rossmann_{date_str}.csv"
    if not fpath.exists():
...
def calculate_inflation(target_date=None, compare_date=None):
    """Calculate inflation metrics for Rossmann.
    ...
    None
        ...
        - ``Inflations/Datas/Cosmetics/Rossmann/rossmann_inflation_<date>.csv``
        - ``Inflations/Datas/Cosmetics/Rossmann/inflation_summary.csv`` (append-only)

    Notes
    -----
    - Matching across dates uses the ``id`` column from the Rossmann scraper.
    - Price column: ``shown_price`` (final price after any discount).
    - Category column: ``category`` (mapped to TUIK groups via
      ``tuik_config.rossmann_category_to_tuik``).
    ...
    # ── Save detailed data ───────────────────────────────────────────────────
    detail_file = INFLATION_OUT_DIR / f"rossmann_inflation_{today_str}.csv"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Rossmann inflation calculator")
    parser.add_argument(
        "--date",
