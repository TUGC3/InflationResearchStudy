...
possible_input_dirs = [
    os.path.join(project_root, "avon_isim_fiyat_cleaned"),
    os.path.join(project_root, "Avon - datas updated"),
    os.path.join(project_root, "avon"),
    os.path.join(project_root, "Avon"),
]
...
if input_dir is None:
    raise FileNotFoundError(
        "Avon input folder bulunamadı. Proje klasörüne şu isimlerden biriyle koy:\n"
        "- avon_isim_fiyat_cleaned\n"
        "- Avon - datas updated\n"
        "- avon\n"
        "- Avon"
    )

output_dir = os.path.join(project_root, "outputs_avon")

summary_output = os.path.join(output_dir, "avon_inflation_summary.csv")
diagnostics_output = os.path.join(output_dir, "avon_diagnostics.csv")
suspicious_output = os.path.join(output_dir, "avon_suspicious_item_changes.csv")
skipped_files_output = os.path.join(output_dir, "avon_skipped_files.csv")
price_matrix_output = os.path.join(output_dir, "avon_product_price_matrix.csv")
...
# Kozmetik datasında kampanya/indirim olabilir.
# Aynı ürünün iki tarih arasında %80'den fazla değişmesi outlier sayılır.
# Kapatmak için None yapabilirsin.
...
for file_path in sorted(all_csv_files):
    ...
    # format_fix_report.csv gibi rapor dosyalarını geç
...
if not df_list:
    print("No valid Avon data found.")
...
for i, current_date in enumerate(available_dates):
    ...
    summary_rows.append({
        ...
        "Genel_Cosmetics_Daily_Inflation": daily_inflation,
        "Genel_Cosmetics_Weekly_Inflation": weekly_inflation
    })
...
print(
    summary_df["Genel_Cosmetics_Daily_Inflation"].min(),
    summary_df["Genel_Cosmetics_Daily_Inflation"].max()
)

print()
print("Weekly inflation min/max:")
print(
    summary_df["Genel_Cosmetics_Weekly_Inflation"].min(),
    summary_df["Genel_Cosmetics_Weekly_Inflation"].max()
)

daily_series = summary_df["Genel_Cosmetics_Daily_Inflation"].dropna()
