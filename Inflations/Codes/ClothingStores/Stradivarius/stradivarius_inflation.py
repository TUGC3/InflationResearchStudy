"""
stradivarius_inflation.py — Stradivarius Daily Price Inflation Calculator

Computes three inflation metrics for Stradivarius Turkey products:
...
Ürün anahtarı: category + product_name + color
...
Input files  : stradivarius_YYYY-MM-DD.csv  (same folder as this script)
Output files :
  - stradivarius_inflation_YYYY-MM-DD.csv  — per-product detail
  - stradivarius_inflation_summary.csv     — store-level summary (one row per day)

Usage:
    python stradivarius_inflation.py
    python stradivarius_inflation.py --date 2026-03-21
    python stradivarius_inflation.py --date 2026-03-21 --compare 2026-03-14
"""
...
def _load_csv(date_str: str) -> pd.DataFrame | None:
    """Load a Stradivarius daily CSV. Returns DataFrame or None."""
    fpath = DATA_DIR / f"stradivarius_{date_str}.csv"
    if not fpath.exists():
...
def calculate_inflation(target_date=None, compare_date=None):
    """Calculate inflation metrics for Stradivarius."""
    ...
    # ── Save detailed CSV ──────────────────────────────────────────────────────
    detail_file = OUTPUT_DIR / f"stradivarius_inflation_{today_str}.csv"
    ...
    summary_file = OUTPUT_DIR / "stradivarius_inflation_summary.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Stradivarius inflation calculator")
