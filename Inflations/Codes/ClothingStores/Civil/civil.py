...
# Civil datasını bu isimlerden biriyle proje klasörüne koyarsan otomatik bulur.
possible_input_dirs = [
    os.path.join(project_root, "civilim_isim_fiyat_cleaned"),
    os.path.join(project_root, "Civil - datas updated"),
    os.path.join(project_root, "Civilim - datas updated"),
    os.path.join(project_root, "Civil"),
    os.path.join(project_root, "civilim"),
]
...
if input_dir is None:
    raise FileNotFoundError(
        "Civil input folder bulunamadı. Proje klasörüne şu isimlerden biriyle koy:\n"
        "- civilim_isim_fiyat_cleaned\n"
        "- Civil - datas updated\n"
        "- Civilim - datas updated\n"
        "- Civil\n"
        "- civilim"
    )

output_dir = os.path.join(project_root, "outputs_civilim")

summary_output = os.path.join(output_dir, "civilim_inflation_summary.csv")
diagnostics_output = os.path.join(output_dir, "civilim_diagnostics.csv")
suspicious_output = os.path.join(output_dir, "civilim_suspicious_item_changes.csv")
skipped_files_output = os.path.join(output_dir, "civilim_skipped_files.csv")
price_matrix_output = os.path.join(output_dir, "civilim_product_price_matrix.csv")
...
# Giyim datasında indirim/kampanya olabileceği için %80 üstü değişimi outlier sayıyoruz.
# Kapatmak istersen None yapabilirsin.
MAX_ABS_ITEM_CHANGE = 0.80

# Weekly hesap için tam 7 gün önce yoksa, 7 gün öncesine en yakın snapshot aranır.
...
if not df_list:
    print("No valid Civil data found.")
...
# Aynı tarihte birden fazla dosya olursa diagnostics için topluyoruz.
source_df = (
...
for i, current_date in enumerate(available_dates):
    ...
    summary_rows.append({
        ...
        "Genel_Clothing_Daily_Inflation": daily_inflation,
        "Genel_Clothing_Weekly_Inflation": weekly_inflation
    })
...
print(
    summary_df["Genel_Clothing_Daily_Inflation"].min(),
    summary_df["Genel_Clothing_Daily_Inflation"].max()
)

print()
print("Weekly inflation min/max:")
print(
    summary_df["Genel_Clothing_Weekly_Inflation"].min(),
    summary_df["Genel_Clothing_Weekly_Inflation"].max()
)

daily_series = summary_df["Genel_Clothing_Daily_Inflation"].dropna()
