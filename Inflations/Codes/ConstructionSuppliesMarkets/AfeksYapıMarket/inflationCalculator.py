...
def dataCompiler(dataA):
    ...
    dataA_test = dataA.groupby(["Product Title"], as_index=False).agg({"Price(TL)": "mean"})

    return dataA_test


def compareData(dataA, dataB):
    data_test = dataA.merge(dataB, how="left", on=["Product Title"], suffixes=("_x", "_y"))
    return data_test
...
def fileInput(fileNew, fileOld):
    ...
    try:
        ...
        df1.rename(columns={"Price": "Price(TL)"}, inplace=True)
    except:
        pass
...
def checkDate(monthNum, dayNum):
    if dayNum > 7:
        test_4 = fileInput(f"Datas\ConstructionProduct{str(monthNum)}-{str(dayNum)}.csv",
                           f"Datas\ConstructionProduct{str(monthNum)}-{str(i - 7)}.csv")
        csvSaver(test_4, "Weekly", 3, dayNum)
    else:
        if (monthNum % 2 == 0 and monthNum <= 7) or (monthNum % 2 == 1 and monthNum > 7) or (monthNum == 8):
            test_4 = fileInput(f"Datas\ConstructionProduct{str(monthNum)}-{str(dayNum)}.csv",
                               f"Datas\ConstructionProduct{str(monthNum - 1)}-{str(24 + monthNum)}.csv")
            csvSaver(test_4, "Weekly", monthNum, dayNum)
            isExtra = True
        else:
            test_4 = fileInput(f"Datas\ConstructionProduct{str(monthNum)}-{str(dayNum)}.csv",
                               f"Datas\ConstructionProduct{str(monthNum - 1)}-{str(23 + monthNum)}.csv")
            csvSaver(test_4, "Weekly", monthNum, dayNum)

    if dayNum > 1:
        test_4 = fileInput(f"Datas\ConstructionProduct{str(monthNum)}-{str(dayNum)}.csv",
                           f"Datas\ConstructionProduct{str(monthNum)}-{str(dayNum - 1)}.csv")
        csvSaver(test_4, "Daily", monthNum, dayNum)
    else:
        if isExtra:
            test_4 = fileInput(f"Datas\ConstructionProduct{str(monthNum)}-{str(dayNum)}.csv",
                               f"Datas\ConstructionProduct{str(monthNum - 1)}-{str(31)}.csv")
            csvSaver(test_4, "Daily", monthNum, dayNum)
        else:
            test_4 = fileInput(f"Datas\ConstructionProduct{str(monthNum)}-{str(dayNum)}.csv",
                               f"Datas\ConstructionProduct{str(monthNum - 1)}-{str(30)}.csv")
            csvSaver(test_4, "Daily", monthNum, dayNum)


for i in range(17, day+8):
    checkDate(month, i)
