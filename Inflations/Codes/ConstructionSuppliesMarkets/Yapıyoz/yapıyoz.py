...
# Yapıyoz datasını bu klasör isimlerinden biriyle proje içine koyarsan otomatik bulur.
possible_input_dirs = [
    os.path.join(project_root, "yapiyoz_isim_fiyat_cleaned"),
    os.path.join(project_root, "Yapiyoz - datas updated"),
    os.path.join(project_root, "Yapıyoz - datas updated"),
    os.path.join(project_root, "Yapiyoz"),
    os.path.join(project_root, "yapiyoz"),
]
...
if input_dir is None:
    raise FileNotFoundError(
        "Yapıyoz input folder bulunamadı. Proje klasörüne şu isimlerden biriyle koy:\n"
        "- yapiyoz_isim_fiyat_cleaned\n"
        "- Yapiyoz - datas updated\n"
        "- Yapıyoz - datas updated\n"
        "- Yapiyoz\n"
        "- yapiyoz"
    )

output_dir = os.path.join(project_root, "outputs_yapiyoz")

summary_output = os.path.join(output_dir, "yapiyoz_inflation_summary.csv")
diagnostics_output = os.path.join(output_dir, "yapiyoz_diagnostics.csv")
suspicious_output = os.path.join(output_dir, "yapiyoz_suspicious_item_changes.csv")
skipped_files_output = os.path.join(output_dir, "yapiyoz_skipped_files.csv")
price_matrix_output = os.path.join(output_dir, "yapiyoz_product_price_matrix.csv")
...
# Yapı market ürünlerinde bazı ürün fiyatları çok yüksek olabilir.
# Ama aynı ürünün bir günde/haftada %80'den fazla değişmesi genelde veri hatası/ürün eşleşme problemi olabilir.
# Kapatmak istersen None yap.
MAX_ABS_ITEM_CHANGE = 0.80

# Weekly hesap için tam 7 gün önce yoksa, 7 gün öncesine yakın snapshot aranır.
...
if not df_list:
    print("No valid Yapıyoz data found.")
...
for i, current_date in enumerate(available_dates):
    ...
    summary_rows.append({
        ...
        "Genel_Yapi_Daily_Inflation": daily_inflation,
        "Genel_Yapi_Weekly_Inflation": weekly_inflation
    })
...
print(
    summary_df["Genel_Yapi_Daily_Inflation"].min(),
    summary_df["Genel_Yapi_Daily_Inflation"].max()
)

print()
print("Weekly inflation min/max:")
print(
    summary_df["Genel_Yapi_Weekly_Inflation"].min(),
    summary_df["Genel_Yapi_Weekly_Inflation"].max()
)

daily_series = summary_df["Genel_Yapi_Daily_Inflation"].dropna()
