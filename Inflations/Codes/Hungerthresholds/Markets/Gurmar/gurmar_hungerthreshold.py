...
FILES = {
    "Feb 2026": f"{BASE_DIR}/gurmar_prices_2026-02-28.csv",
    "Mar 2026": f"{BASE_DIR}/gurmar_prices_2026-03-31.csv",
    "Apr 2026": f"{BASE_DIR}/gurmar_prices_2026-04-30.csv",
    "May 2026": f"{BASE_DIR}/gurmar_prices_2026-05-31.csv",
}
...
MATCH_RULES = {
    ...
    "Fish": {
        ...
        "exclude": ["Füme", "Konserve", "Kedi", "Köpek", "Maması", "Sosu", "Soslu",
                    ...
                    "Onigiri", "Sushida", "Fish Finger", "Mister No"],
    ...
    "Eggs": {
        ...
        "exclude": ["Organik", "Bıldırcın", "Toz", "Akı", "Sarısı", "Şeker", "Haribo",
                    ...
                    "Yumurtalı", "Ozmo", "Ülker", "Mini","Kinder"],
        "unit": "piece",
    },
    "Chickpeas": {
        "keywords": ["Nohut"],
    ...
    "Onion": {
        ...
        "exclude": ["Taze", "Pırasa", "Toz", "Pul", "Sarımsak", "Halkası", "Cips",
                    ...
                    "Nutzz", "Çizi", "Baharat", "Cipsi", "Eti Crax"],
        "unit": "kg",
    },
    "Eggplant / Zucchini": {
...
def load_file(path: str) -> pd.DataFrame:
    ...
    for candidate in ['product_name', 'product-name']:
        if candidate in df.columns:
            df = df.rename(columns={candidate: 'name'})
            break
    if 'name' not in df.columns:
        raise ValueError(f"No product name column in {path}. Columns: {df.columns.tolist()}")
    df['price']     = df['price'].apply(parse_price_str)
    df['is_per_kg'] = False
...
def compute_hunger_threshold(csv_path: str, date_label: str) -> pd.DataFrame:
    ...
    if df.empty:
        print(f"  ⚠  {date_label}: file is empty, skipping.")
        return pd.DataFrame()
